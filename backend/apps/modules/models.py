...
def generate_upload_path(instance, filename):
    ...
    if hasattr(instance, 'task_file') and instance.task_file.name == filename:
        return f"exams/tasks/{new_filename}"
    elif hasattr(instance, 'description_file') and instance.description_file.name == filename:
        return f"exams/descriptions/{new_filename}"
    return f"uploads/{new_filename}"


def task_file_path(instance, filename):
    ext = filename.split('.')[-1]
    return f"exams/tasks/{uuid.uuid4().hex}.{ext}"


def description_file_path(instance, filename):
    ext = filename.split('.')[-1]
    return f"exams/descriptions/{uuid.uuid4().hex}.{ext}"


def module_file_path(instance, filename):
    ext = filename.split('.')[-1]
    return f"modules/files/{uuid.uuid4().hex}.{ext}"


def step_image_path(instance, filename):
    ext = filename.split('.')[-1]
    return f"modules/steps/{uuid.uuid4().hex}.{ext}"


class ExamVariant(models.Model):
    code = models.CharField('Код экзамена', max_length=50, help_text='Например: КОД 1.2')
    variant_number = models.PositiveIntegerField('Номер варианта')
    name = models.CharField('Название', max_length=300)
    description = models.TextField('Описание предметной области', blank=True)
    description_file = models.FileField(
        'PDF описания',
        upload_to=description_file_path,
    ...
    total_time = models.CharField(
        ...
        max_length=50,
        blank=True,
        help_text='Например: 6 часов 30 минут'
    )
    ...
    def __str__(self):
        return f"{self.code} Вариант {self.variant_number}: {self.name}"
...
class Section(models.Model):
    ...
    session_number = models.PositiveIntegerField('Номер сессии', null=True, blank=True)
    order = models.PositiveIntegerField('Порядок', default=0)
    is_active = models.BooleanField('Активен', default=True)
    created_at = models.DateTimeField('Создан', auto_now_add=True)
...
class Module(models.Model):
    ...
    task_condition = models.TextField(
        'Условие задания',
        blank=True,
        help_text='Текст задания из PDF экзамена'
    )
    task_number = models.CharField(
        'Номер задания',
        max_length=20,
        blank=True,
        help_text='Например: 1.1, 2.3'
    )
...
class ModuleFile(models.Model):
    ...
    description = models.TextField('Описание', blank=True)
    file = models.FileField('Файл', upload_to=module_file_path, storage=get_raw_storage)
...
class StepImage(models.Model):
    ...
    image = models.ImageField('Изображение', upload_to=step_image_path)
